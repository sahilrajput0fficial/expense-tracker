...
if ($method == 'GET') {
    ...
    if($insert->execute()){
        ...
        sendResponse(200, ["id" => $expense_id, "frequency" => $frequency]);
    } else {
    ...
    $update = $conn->prepare("UPDATE expenses SET expense_name=?, amount=?, category=?, notes=?, payment_method=? WHERE id=?");
    $update->bind_param("sdsssi", $expense_name, $amount, $category, $notes, $payment_method, $id);
